...
\begin{itemize}
  ...
  \item Finally for any resource ``r'', if the size of the set $| C(x) : x \in A_r | > 10 \quad or \quad | C(x) : x \in B_r | > 10$, we discard \emph{r} and its corresponding experiment pairs from \emph{E}, since the resource modified/deleted by too many families is less interesting.
