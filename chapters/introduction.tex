...
We analyze large number of malware samples and find possible malware pairs from those samples, that could exhibit behavioral interference.
...
% We call those malware pairs as possible \textbf{candidate pairs}.
...
The simple heuristic of finding interfering malware candidates based on common resource gave us concrete candidate pairs.
...
Both malware sample belonging to a candidate pair were run in the \emph{Anubis} system consecutively, with a time interval between the two run.
We describe the process of running the candidate pair in \emph{Anubis} system in~\autoref{sub:Running the Candidate}.
The result of Anubis was then analyzed to see if there was deletion of resources as expected, which proves for behavioral interference between the two malware.
The details of result analysis is describe in~\autoref{sub:Result Analysis}.\\
