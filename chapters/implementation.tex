...
\end{itemize}
Of course combinations and nodes that do not have a path from source to sink should not be put in the graph (there are some in my example)
\begin{figure}[htbp]
  \centering
...
The relations can be represented in the graph below, where, I is a set of input clusters from set \emph{``A''} and \emph{``O''} is the output clusters from set B. Set A, B, and R has usual meanings as described in the\textit{~\nameref{chapter:methedology} Section}.
...
% TODO: put algorithm pseudo code
