...
\section{Study of Anubis and Database}
...
We had to effectively and efficiently analyze those data for the goal of our research.\\
We started with studying the Anubis system and current database.
We primarily dealt with two database of Anubis backend, the \textbf{`db\_report'} and \textbf{'web\_analysis'}.
The \emph{`web\_analysis'} database was hit first for any binary submitted for analysis.
It consisted of tables such as \emph{`result', `file', and `file\_task'}.
Each submission was a unique row in \emph{`result'} table.
A new row is created in \emph{`file'} table with the \emph{`md5'} and \emph{`sha'} hash of the binary.
We refer the unique primary key of the \emph{`file'} table as \textbf{malware id}.
The \emph{file} and \emph{`result'} tables was associated through \emph{`file\_task'} table.
...
For each resource type, their \textit{create, read, delete} resource activities were extracted.
...
Since the number of malware were too large trying to save all their activities in data structure would cause our machine to run out of memory and crash the system.
To solve this we took the approach of map reduce.\\
We worked on a batch of 50K malware at a time and saved the reverse index of activity to the file with a numbering.
Around 420 files were created for each resource type. The result files were in the format where resource name and list of unique malware id were separated by commas (,) as delimiter.
This was one of the time consuming operation and it took almost 5 days for the script to finish.
After the reverse index was generated in multiple numbered output, we sorted the file based on resource name, alphabetically.
We joined the different parts with regard to the resource activity as key to merged it into one single file.\\
...
\end{figure}
\\
We used a struct of 3 integer size to read and hold the meta information. The size of last three \textit{unsigned int} byte had the binary pair sizes information and delay time, known as meta information.
...
Then these bytes were saved to the new file pointer. Once the binary pairs were extracted by the unpacker new binary files were created in the Anubis environment and those were executed one after another with the delay of time.
We used \emph{windows.h} standard libraries \emph{CreateProcess} and \emph{Sleep} function for the purpose.\\
...
We looked for a set $(a, b)$, where malware `a' creates the resource \emph{r}, and malware `b' deletes the same resource, \emph{r}, such that no other malware has create or delete activities on that resource \emph{r}.\\
After finding such pairs, when we ran those malware pairs in the Anubis system using the \emph{unpacker}.\\
...
We used \emph{anubis} log file and the behavioral profiles, as described in~\autoref{sub:Behavioral Profile}, to find such failed activities of deletion and access.
...
% We had all the resource activities such as modification, access and deletion related to different resource types.
% We represented each resource type and activities as numeric value. And each resource name would be represented by it's unique resource name id from our new database.
...
$90\%$ of the cluster, about \emph{629}, falls under the cluster size of less than 50,000 and \emph{4 million} malware, which is the $65\%$ of total malware, are in the cluster size of less than 50K.\\
...
% We represented malware with its \emph{`md5'} hash.
